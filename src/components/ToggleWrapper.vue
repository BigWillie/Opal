<template>
      <toggle @state-change="stateChange($event)" :on="currentState" :label="label">
      <template v-slot="{toggle, active, label}">
            ...
            <label class="flex items-center cursor-pointer">
                ...
                <div class="relative">
                    ...
                    <div class="hidden-wrap">
                        <input :checked="active" type="checkbox" @change="toggle()" />
                    </div>
                    <!-- line -->
                    <div :class="{'toggle-line-on' : active}" class="toggle-line w-10 h-4 bg-gray-400 rounded-full shadow-inner"></div>
                    <!-- dot -->
                    <div :class="{'toggle-dot-on' : active}" class="toggle-dot absolute w-6 h-6 bg-white rounded-full shadow inset-y-0 left-0"></div>
                </div>
                <!-- label -->
